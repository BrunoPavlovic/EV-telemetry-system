...
public class VoznjeDAO {
  ...
  public List<Vozilo> dohvatiVoznje(long odVremena, long doVremena) {
    String upit = "SELECT id, broj, vrijeme, brzina, snaga, struja, visina, gpsBrzina, tempVozila,"
        ...
        + "        gpsSirina, gpsDuzina " + "FROM voznje WHERE vrijeme >= ? AND vrijeme <= ?";
  ...
  /**
   * Dohvati voznje vozila.
   *
   * @param id id vozila
   * @return lista voznji
   */
  public List<Vozilo> dohvatiVoznjeVozila(int id) {
    String upit = "SELECT id, broj, vrijeme, brzina, snaga, struja, visina, gpsBrzina, tempVozila,"
        + "postotakBaterija, naponBaterija, kapacitetBaterija, tempBaterija, preostaloKm, ukupnoKm,"
        + "gpsSirina, gpsDuzina" + "FROM voznje WHERE id = ?";
   ...
   */
  public List<Vozilo> dohvatiVoznjeVozila(int id, long odVremena, long doVremena) {
    String upit = "SELECT id, broj, vrijeme, brzina, snaga, struja, visina, gpsBrzina, tempVozila,"
        + "postotakBaterija, naponBaterija, kapacitetBaterija, tempBaterija, preostaloKm, ukupnoKm,"
        + "gpsSirina, gpsDuzina FROM voznje WHERE id = ? AND vrijeme >= ? AND vrijeme <= ?";
  ...
  /**
   * Dodaj voznju za e-vozilo
   * 
   * @param vozilo
   * @return true, ako je dodavanje uspjesno
   */
  public boolean dodajVoznju(Vozilo vozilo) {
    String upit =
        "INSERT INTO voznje(id, broj, vrijeme, brzina, snaga, struja, visina, gpsBrzina, tempVozila, postotakBaterija, naponBaterija, kapacitetBaterija, tempBaterija, preostaloKm, ukupnoKm, gpsSirina, gpsDuzina) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
